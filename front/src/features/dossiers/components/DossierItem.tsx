...
const DossierItem = ({
  ...
  return (
    <div className="flex items-center justify-between py-3 pl-5 pr-3 w-full hover:bg-bg-secondary transition-colors cursor-pointer relative group">
      {!loading ? (
        <>
          ...
          <div className="w-[120px] flex justify-end items-center gap-2">
            {status === 'À venir' ? <StatusBadge status="À compléter" /> : <StatusBadge status={status} />}
          </div>
        </>
      ) : (
        <>
      ...
      {/* Actions — toujours visible, opacité réduite hors hover */}
      <div ref={popoverRef} className="absolute right-3 top-1/2 -translate-y-1/2 z-10">
        <button
          onClick={(e) => {
            e.stopPropagation();
            setIsPopoverOpen(!isPopoverOpen);
          }}
          className="p-1.5 rounded-md transition-all opacity-40 group-hover:opacity-100 hover:bg-bg-tertiary hover:opacity-100"
          aria-label="Actions"
        >
          <DotsVertical className="w-4 h-4 text-text-secondary" />
        </button>

        {isPopoverOpen && (
          <div className="absolute right-0 top-full px-1 mt-1 w-44 bg-bg-primary rounded-[var(--radius-lg)] shadow-lg border border-border py-1">
            <button
              onClick={(e) => {
                e.stopPropagation();
                if (onEdit) {
            ...
            <button
              ...
              className="w-full rounded px-4 py-2 text-left hover:bg-red/15 flex items-center gap-2 transition-colors"
            >
