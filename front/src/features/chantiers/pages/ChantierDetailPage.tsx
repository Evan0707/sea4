...
export const ChantierDetailPage = () => {
  ...
  const handleAssignArtisan = async (artisan: Artisan) => {
    ...
    try {
      // Use the existing PUT endpoint for updateEtapes
      // Note: Endpoint expects an array of etapes
  ...
  return (
    <div className="p-8">
      <button
        onClick={() => navigate(-1)}
        className="flex items-center gap-2 text-placeholder hover:text-primary transition-colors mb-6"
      >
          ...
          {chantier.etapes.length > 0 ? (
            <div className="space-y-2 max-h-[400px] overflow-y-auto pr-2">
              {sortedEtapes.map((etape) => {
                const canStart = chantier.statut === 'En chantier' && etape.statut === 'À venir' && !etape.reservee;
                ...
                const previousCompleted = !previousEtape || previousEtape.statut === 'Terminée' || previousEtape.reservee;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
        ...
        <Card>
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              <H2 className="text-lg">Factures (Artisans)</H2>
            </div>
