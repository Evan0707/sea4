 ...
 */
export const getAllDepartements = (): Departement[] => {
  return DEPARTEMENTS;
};
 ...
 */
export const getDepartementsByRegion = (region: string): Departement[] => {
  return DEPARTEMENTS.filter(dept => dept.region === region);
};

/**
 * Récupère toutes les régions uniques
 */
export const getAllRegions = (): string[] => {
  return Array.from(new Set(DEPARTEMENTS.map(dept => dept.region))).sort();
};

/**
 * Recherche des départements par nom ou code
 */
export const searchDepartements = (query: string): Departement[] => {
  const lowerQuery = query.toLowerCase();
  return DEPARTEMENTS.filter(dept => 
    dept.nom.toLowerCase().includes(lowerQuery) || 
