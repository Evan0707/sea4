...
try {
    ...
    if (empty($toDrop)) {
        echo "No non-CASCADE constraints found for batiparti.etape_chantier(nochantier).\n";
        exit(0);
    }

    echo "Dropping constraints: " . implode(', ', $toDrop) . "\n";
    foreach ($toDrop as $cname) {
        $sqlDrop = sprintf('ALTER TABLE batiparti.etape_chantier DROP CONSTRAINT %s', $cname);
